...
@section('title', 'Praktikum Dibatalkan')

@section('header')
    @include('components.navs.admin', ['page' => "cancelled-practices"])
@endsection
...
<script>
    function
     getForms() {
        ...
        fetch(`{{ route('api-practice-plan-cancelled-list') }}?date=${practiceDate}`)
    ...
    function acceptForm(id) {
        fetch("{{ route('api-practice-plan-cancelled-acc') }}", {
            method: "POST",
            headers: {
